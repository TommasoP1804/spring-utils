 ...
 * @since 3.0.0
 */
fun ServerRequest.headerOrThrow(name: String, `class`: KClass<*> = List::class, lazyException: ThrowableSupplier = { RequiredHeaderException(name, `class`) }): List<String> =
 ...
 * @since 3.0.0
 */
fun ServerRequest.headerOrThrow(name: String, `class`: KClass<*> = String::class, internalErrorCode: String): List<String> =
...
fun ServerRequest.paramOrThrowAsStringList(name: String) = paramOrThrow(name, List::class).splitAndTrim(Char.COMMA)
...
fun ServerRequest.paramOrDefaultAsStringList(name: String, defaultValue: Supplier<List<String>>) = paramOrNull(name)?.splitAndTrim(Char.COMMA) ?: defaultValue()
