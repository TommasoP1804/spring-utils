...
fun <T : Any> conditionalGet(
    ...
    if (includeRequestId) RequestIdProvider.requestIdThreadLocal.get().ifNotNull { response.header(HttpHeader.REQUEST_ID, it.toString()) }
...
fun EmptyResponse(
    ...
    if (includeRequestId) RequestIdProvider.requestIdThreadLocal.get().ifNotNull { re.header(HttpHeader.REQUEST_ID, it.toString()) }
...
internal fun generateMultiStatusMap(results: List<ResourceResult>, httpVersion: String): List<DataMap> {
    ...
    for ((reference, statusCode, properties) in results) {
        val map: DataMMap = emptyMMap()
        map["reference"] = reference
        map["status"] = "HTTP/" + httpVersion.substringAfter("HTTP/") + Char.SPACE + statusCode.value + Char.SPACE + statusCode.reasonPhrase
        if (properties.isNotNullOrEmpty) map["properties"] = properties
        list += map
    }
    return list
}
...
internal fun generateMultiStatusXML(results: List<ResourceResult>, httpVersion: String) = buildString {
    ...
    for ((reference, statusCode, properties) in results) {
        append("  <d:response>\n")
        append("    <d:href>${reference.escapeXml()}</d:href>\n")
        append("    <d:propstat>\n")
        append("      <d:prop>\n")
        ...
        append("      <d:prop>\n")
        append("      <d:status>HTTP/${httpVersion.substringAfter("HTTP/")} ${statusCode.value} ${statusCode.reasonPhrase}</d:status>\n")
        append("    </d:propstat>\n")
        append("  </d:response>\n")
    }
 ...
 * @since 2.0.9
 */
fun ResponseEntity.BodyBuilder.requestId(id: RequestId) = header(HttpHeader.REQUEST_ID, id.toString())
...
/**
 * Sets the `Content-Type` header for the response being built.
 *
 * @param mediaType The `MediaType` instance to set as the value of the `Content-Type` header.
 * @since 3.6.1
 */
fun ResponseEntity.BodyBuilder.contentType(mediaType: MediaType) = contentType(mediaType.toSpringMediaType())
