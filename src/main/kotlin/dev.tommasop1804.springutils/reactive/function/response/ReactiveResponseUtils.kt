...
suspend inline fun <reified T : Any> conditionalGet(
    ...
    val eTagNoneMatch = request.ifNoneMatch().getOrNull().orEmpty()
    val ifModifiedSince = request.ifModifiedSince().getOrNull()?.toOffsetDateTime()
    ...
    if (status == HttpStatus.NotModified) return response.buildAndAwait()
...
suspend inline fun <T : Any, reified R : Any> conditionalUpdate(
    ...
    val eTagIfMatch = request.ifMatch().orEmpty()
    val ifUnmodifiedSince = request.ifUnmodifiedSince().getOrNull()?.toOffsetDateTime()
