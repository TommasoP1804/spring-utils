...
/**
 * Converts a Spring `HttpMethod` instance to a `HttpMethod` instance from KUtils.
 *
 * This extension function maps the `name` of the current `HttpMethod`
 * instance from the Spring framework to a corresponding `HttpMethod`
 * in KUtils, ensuring type compatibility between the two frameworks.
 *
 * @receiver The `HttpMethod` instance from Spring framework.
 * @return The corresponding `HttpMethod` instance from KUtils or `null`.
 * @since 5.1.0
 */
fun org.springframework.http.HttpMethod.toKutilsHttpMethod() = HttpMethod of name()

/**
 * Converts a custom [dev.tommasop1804.kutils.classes.web.HttpStatus] instance to a Spring Framework's [HttpStatus].
 * If the conversion fails, it safely returns `null`.
 *
 * @receiver The custom [dev.tommasop1804.kutils.classes.web.HttpStatus] instance to convert.
 * @return The corresponding Spring [HttpStatus] if the conversion is successful; otherwise `null`.
 */
fun dev.tommasop1804.kutils.classes.web.HttpStatus.toSpringHttpStatus() = tryOrNull{ HttpStatus.valueOf(value) }
/**
 * Converts an instance of `org.springframework.http.HttpStatus` to its corresponding
 * `dev.tommasop1804.kutils.classes.web.HttpStatus` representation.
 *
 * The conversion is performed using the numeric status code value of the input HTTP status.
 *
 * @receiver The `org.springframework.http.HttpStatus` object to be converted.
 * @return The corresponding `dev.tommasop1804.kutils.classes.web.HttpStatus` representation.
 */
fun HttpStatus.toKutilsHttpStatus() = dev.tommasop1804.kutils.classes.web.HttpStatus.of(value())
 ...
 * The method attempts to extract and parse the value associated with `HttpHeader.REQUEST_ID`.
 * If the header is missing or if there is an error during parsing, an `IterableError.NotFound` is returned.
 *
 * @return Either a `RequestId` instance containing the parsed request identifier,
 * or an `IterableError.NotFound` if the `HttpHeader.REQUEST_ID` is not found.
 * @since 5.1.0
 */
fun HttpHeaders.getRequestId(): Either<IterableError.NotFound, RequestId> = getOrError(HttpHeader.REQUEST_ID)
    .thenMergeWith { it.firstOrError() }
    .mapLeft { IterableError.NotFound(HttpHeader.REQUEST_ID) }
    .map(::RequestId)
/**
 ...
 * @since 3.5.0
 */
@JvmName("getPropertyOrThrowLazyMessage")
...
/**
 * Attempts to retrieve the property value associated with the given key.
 * Returns the property value wrapped in an `Either` if found, or a `NotFound` object if not found.
 *
 * @param key The key of the property to retrieve.
 * @return An `Either` containing the property value as a string if successful,
 *         or a `NotFound` instance if the property is not found.
 * @since 5.1.0
 */
fun PropertyResolver.getPropertyOrError(key: String): Either<NotFound, String> =
    tryOrError({ NotFound(key) }) { getPropertyOrThrow(key) }
/**
 * Attempts to retrieve a property from the PropertyResolver and cast it to the specified type.
 * If the property retrieval fails due to an invalid format or missing property, it returns an error wrapped in an Either type.
 *
 * Possible errors:
 * - [InvalidTypeFormat] - The property value could not be cast to the specified type.
 * - [NotFound] - The property was not found.
 *
 * @param key The name of the property to retrieve.
 * @return An Either containing either the successfully retrieved property cast to the specified type,
 *         or an Error if the property cannot be retrieved or cast.
 * @since 5.1.0
 */
@JvmName("getPropertyOrErrorTyped")
inline fun <reified T : Any> PropertyResolver.getPropertyOrError(key: String): Either<Error, T> =
    tryOrError(
        { InvalidTypeFormat(key, typeOf<T>(), it.message) },
        specificCases = NoSuchEnvPropertyException::class mapWith { NotFound(key) }
    ) { getPropertyOrThrow<T>(key) }
 ...
 */
context(resolver: PropertyResolver)
fun String.resolvePlaceholdersOrThrow(lazyException: ExceptionSupplier = { NoSuchEnvPropertyException() }) = tryOrThrow(
